<?php
    ...
    $valid_columns = [
        'BidTitle', 'SolicitationID', 'RelatedNotice', 'BidLink', 'Location', 'DepartmentNames', 
        'SetAsideCode', 'ServiceCode', 'NAICS', 
    ];

    $sort_column = isset($_GET['sort']) && in_array($_GET['sort'], $valid_columns) ? $_GET['sort'] : 'BidTitle';
    ...
    $total_result = $conn->query("SELECT COUNT(*) AS total FROM finalized_sam_gov WHERE $where_sql");
        ...
        <table>
            ...
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    ...
                    foreach ($valid_columns as $column) {
                        if ($column === 'BidLink') {
                            echo "<td><a href='" . htmlspecialchars($row[$column]) . "' target='_blank'>Bid Link</a></td>";
                        } else {
                ...
                echo "<tr><td colspan='12'>No bids found with those parameters.</td></tr>";
            }
            $conn->close();
            ?>
        </table>
