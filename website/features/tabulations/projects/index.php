<?php
    ...
    $valid_columns = [
        'project_number', 'project_name', 'awarding_body', 'project_id_number',
        'project_dir_number', 'project_description',
        'date_started', 'date_finished', 'city', 'complete_address', 'county'
    ];

    $sort_column = isset($_GET['sort']) && in_array($_GET['sort'], $valid_columns) ? $_GET['sort'] : 'project_number';
    ...
    $total_result = $conn->query("SELECT COUNT(*) AS total FROM dir_projects WHERE $where_sql");
    ...
    <form method="GET" id="searchForm">
        <table>
            ...
            if ($result->num_rows > 0) {
                ...
                echo "<tr><td colspan='" . count($valid_columns) . "'>No entries found with those parameters.</td></tr>";
            }
            $conn->close();
            ?>
        </table>
