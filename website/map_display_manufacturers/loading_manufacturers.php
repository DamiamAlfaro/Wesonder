...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql = "SELECT * FROM manufacturers WHERE spec_number LIKE '$checkboxValue%'";
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            $manufacturers[] = [
                ...
                "spec_complete_name" => $row['spec_complete_name'],
                "manufacturer_name" => $row['manufacturer_name'],
                "manufacturer_address" => $row['manufacturer_address'],
                "manufacturer_phones" => $row['manufacturer_phones'],
