...
# [1.1] Organization is nice and it
# makes you feel in control you know
...
# bids from the website. We are merely
# webscriping the individual
# solicitation numbers in order
# to individually expropriate each of
# their attributes separately
def webscraping_piee_website(piee_url):
    ...
    # [1.1] This is purely derived
    # from a desire for organization...
...
# [2.0] Expropriating each individual
# link from Step [1]
...
# [3.2] Design of data is also a crucial
# factor when it comes of organization
...
def acquiring_geolocations(list_of_bids):
    ...
    for bid in list_of_bids:
        ...
        if geolocations[0] != 0:
            ...
            # [3.2] Cleansing those zip codes
            new_string = split_string_on_five_digits(other_address)
            new_string = " ".join(
                new_string.split(" ")[1:]
            )

            # [3.1.1] Acquiring the geolocations with the newly
            # cleansed zip codes, round 2
...
# [1] PIEE Webscraping based on given index, the result
# of such function is the individual solicitation numbers
# that will be use to expropriate each individual
# bid in the future on Step [2]
piee_url = 'https://piee.eb.mil/sol/xhtml/unauth/index.xhtml'
piee_active_bids_urls = webscraping_piee_website(piee_url)

# [2] Expropriation of bid attributes
piee_bids = piee_active_bids_webscraping(piee_active_bids_urls)

# [3] Bids geolocation expropriation
updated_active_bids = acquiring_geolocations(piee_bids)

# Convert to csv file
df = pd.DataFrame(updated_active_bids)
headers = [
    "functional_url",
    "solicitation_value",
    "notice_type_element",
...
df.columns = headers
df.to_csv('finalized_piee_bids.csv', mode="w",index=False)
